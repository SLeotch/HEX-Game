...
function genereDamier(rayon, nbLignes, nbColonnes) {
    ...
    d3.select("#tablier").append("svg").attr("width", largeur).attr("height", longueur);
    ...
    for (var ligne = 0; ligne < nbLignes; ligne++) {
        for (var colonne = 0; colonne < nbColonnes; colonne++) {
            ...
            for (var h = 0; h < hexagone.length; h++) {
                ...
                y = distance*2 + hexagone[h][1]+(rayon-distance*2)*(1+2*ligne) + 40;
                d += x + "," + y + " ";
            }
            ...
            d3.select("svg").append("polygon")
                ...
                .on("click", function () {
                    ...
                    const numHexagone = parseInt(d3.select(this).attr('id').substring(1));
                    const nomJoueur = document.getElementById('nom').value;
                    if(nomJoueur){
                        socket.emit('selectionHexagone', { numJoueur: nomJoueur, numHexagone });
                    }
                });
